...
import {IsBoolean, IsEnum, IsNotEmpty, IsString, Length, Matches} from "class-validator";
...
export class CreateUserDto {
    ...
    @Length(8, 16, { message: 'username length should be between 8 and 16 characters' })
    ...
    @Length(8, 24, { message: 'password length should be between 8 and 24 characters' })
    @Matches(
        /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]+$/,
