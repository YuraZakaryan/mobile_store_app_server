...
import {FindOneParams, TCategoryUpdateData, TProductUpdateData} from "../types";
import {TReturnItem} from "../user/types";

@Injectable()
export class ProductService {
    constructor(@InjectModel(Product.name) private productModel: Model<Product>, @InjectModel(Category.name) private categoryModel: Model<Category>, private fileService: FileService) {
    }

    async create(dto: CreateProductDto, picture: Express.Multer.File): Promise<Product> {
        ...
        return await product.populate('category author')
    }

    async update(params: FindOneParams, dto: CreateProductDto, picture: Express.Multer.File): Promise<Product> {
        const id = params.id
        ...
        if (updatedProduct && dto.category.toString() !== oldProduct.category.toString()) {
        ...
        return updatedProduct.populate('category author')
    }

    async getAll(limit?: number, skip?: number): Promise<TReturnItem<Product[]>> {
